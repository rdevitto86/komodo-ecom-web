...
export default class NumberExtended {
     ...
     * @returns {Number} whole number (int)
     */
    static toWholeNumber(d) {
        if (typeof d !== 'number') {
            return NaN;
        }
     ...
     * @returns {Number} demical value (float)
     */
    static toDecimal(w) {
        if (typeof w !== 'number') {
            return NaN;
     ...
     * @returns {Number} original number
     * @example
     * let og = 145.56, fv = 4.36, p = 0.03
     * revertPercentage(fv, p) => 145.56
     */
    static revertPercentage(fv, p) {
